      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            {(['gemini', 'mistral'] as const).map((provider) => (
              <button
                key={provider}
                type="button"
                onClick={() => setFormData({ ...formData, primaryProvider: provider })}
                className={`flex-1 brutal-border p-3 text-center font-bold capitalize transition-all ${
                  formData.primaryProvider === provider

...
import { searchCourses, generateRoadmap } from '@/services/api';
import { useAISettings } from '@/context/AISettingsContext';

function parseError(err: unknown): string {
  const msg = err instanceof Error ? err.message : String(err);
  if (msg.includes('rate') || msg.includes('429') || msg.includes('quota') || msg.includes('rate-limited')) {
    return 'Gemini rate limit reached. Please wait ~30 seconds and try again.';
  }
  if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
    return 'Cannot reach the server. Make sure "npm run dev" is running.';
  }
...
export function useCourseSearch() {
  ...
  return { courses, isLoading, error, search, clearResults };
}
...
export function useRoadmap() {
  ...
  return { roadmap, isLoading, error, generate, clearRoadmap };
}
